...
VIDEO_PATH = os.path.join(OUTPUT_DIR, "disco_voador_multifluxo.mp4")
...
points['speed'] = np.linalg.norm(vel, axis=1)
points['energy'] = 0.5 * points['speed']**2
plotter.add_points(points, scalars='energy', cmap='inferno', point_size=3, clim=[0, 60])
...
def update(frame):
    ...
    plotter.add_mesh(disco, color='silver', metallic=True, roughness=0.1)
    plotter.add_points(points, scalars='energy', cmap='inferno', point_size=3, clim=[0, 60])
    plotter.add_text(f"Multifluxo 3D | Frame {frame}", position='upper_left', color='white', font_size=12)
    plotter.write_frame()
